(() => {
  ...
  function persist(){clearTimeout(saveTimer);if(!state.remember){localStorage.removeItem('homecoming-scene');$('#saveState').textContent='本次设置不保存';return;}$('#saveState').textContent='正在保存…';saveTimer=setTimeout(()=>{localStorage.setItem('homecoming-scene',JSON.stringify(state));$('#saveState').textContent='已保存到本机';},220);}
  ...
  async function render(){
    ...
    for(const layer of state.layers){
      ...
      bindLayer(el,layer,handle);canvas.append(el);
    }
    scene.classList.toggle('motion',state.motion); $('#welcomeHotspot').hidden=!state.hotspots;$('#waterHotspot').hidden=!state.hotspots;
    syncControls();
  }
  function bindLayer(el, layer, handle){
    let drag=null, moved=false;
    el.addEventListener('pointerdown',e=>{if(e.target===handle)return; e.preventDefault(); select(layer.id); moved=false; const p=pointerPct(e);drag={x:p.x-layer.x,y:p.y-layer.y};el.setPointerCapture(e.pointerId);});
    el.addEventListener('pointermove',e=>{if(!drag)return;const p=pointerPct(e);if(Math.abs(p.x-drag.x-layer.x)>1||Math.abs(p.y-drag.y-layer.y)>1)moved=true;layer.x=clamp(p.x-drag.x,-layer.w*.7,100-layer.w*.3);layer.y=clamp(p.y-drag.y,-layer.h*.7,100-layer.h*.3);el.style.left=`${layer.x}%`;el.style.top=`${layer.y}%`;});
    el.addEventListener('pointerup',e=>{if(drag){drag=null;persist();if(!moved)triggerLayer(layer,el);}});
  ...
  function syncControls(){
    ...
    if(l){$('#selectedName').textContent=l.name;$('#opacityRange').value=Math.round(l.opacity*100);$('#opacityValue').value=`${Math.round(l.opacity*100)}%`;$('#layerAction').value=l.action||'none';}
    $('#speechText').value=state.speechText;$('#rateRange').value=state.rate;$('#rateValue').value=state.rate;$('#pitchRange').value=state.pitch;$('#pitchValue').value=state.pitch;$('#audioLoop').checked=state.loop;$('#showHotspots').checked=state.hotspots;$('#motionToggle').checked=state.motion;$('#rememberToggle').checked=state.remember;$('#saveState').textContent=state.remember?'已保存到本机':'本次设置不保存';
