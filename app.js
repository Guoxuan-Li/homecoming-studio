(() => {
  ...
  function bindLayer(el, layer, handle){
    ...
    el.addEventListener('pointerdown',e=>{if(e.target===handle)return;e.preventDefault();select(layer.id);moved=false;pointers.set(e.pointerId,{x:e.clientX,y:e.clientY});el.setPointerCapture(e.pointerId);if(pointers.size===1){const p=pointerPct(e);drag={x:p.x-layer.x,y:p.y-layer.y};}else if(pointers.size===2){gesture={angle:angle(),rotation:layer.r||0};drag=null;moved=true;}});
